function total(buku, diskon, pajak, stock, pemesanan, kredit){
    let result = 0;
    ...
    console.log();
    console.log();
    console.log('Dikredit ', kredit, 'Bulan');
    console.groupEnd();

    let Tempo = result/kredit;
    let bayar = [];
    for(let index=0; index < kredit; index++){
        bayar.push(
            {
                Angsuran_ke: index + 1,
                Rp: Tempo
    ...
    console.log(Array.from(bayar));
    return result;

} 

total({judul: "Manga One Piece", harga: 20000000}, 15, 5, 3, 3, 12);